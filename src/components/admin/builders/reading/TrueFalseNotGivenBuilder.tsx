...
import type { TrueFalseNotGivenQuestion, TrueFalseStatement, AnswerKey } from '@/types';

interface Props {
  question: TrueFalseNotGivenQuestion;
  onChange: (next: TrueFalseNotGivenQuestion) => void;
}

type TFNValue = 'true' | 'false' | 'not_given';

export default function TrueFalseNotGivenBuilder({ question, onChange }: Props) {
  ...
  const [answers, setAnswers] = useState<Record<number, TFNValue>>({});

  useEffect(() => {
    const nextContent: TrueFalseNotGivenQuestion['content'] = {
      ...question.content,
      questionText,
      instructions,
      statements,
  ...
  const setAnswer = (id: number, value: TFNValue) => {
          ...
          {statements.map((s, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  value={answers[s.id] ?? ''}
                  onChange={(e) => setAnswer(s.id, e.target.value as TFNValue)}
